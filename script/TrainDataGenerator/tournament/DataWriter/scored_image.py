...
class WriteThread(Thread):
    def __init__(self, scored_player_list: list, save_dir_path: str):
    ...
    def run(self):
        for scored_player in self.scored_player_list:
            ...
            image.save(str(self.save_dir_path/f'{scored_player["score"]:0=3}.png'))
...
class ScoredImageWriter(DataWriter):
    def __init__(self, save_dir_path: str):
        ...
        if not self.save_dir_path.is_dir():
            raise ScoredImageWriterException(f'path is not dir')

    def write(self, scored_player_list: list):
        write_thread = WriteThread(scored_player_list, str(self.save_dir_path))
        write_thread.start()
